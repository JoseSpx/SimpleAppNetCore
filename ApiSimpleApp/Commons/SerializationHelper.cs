...
namespace ApiSimpleApp.Commons
{
    ...
    public static class SerializationHelper
    {
        ...
        public static string SerializeToXMLToList<T>(this List<T> value)
        {
            if (value == null)
            {
                return string.Empty;
        ...
        public static List<T> DeserializeFromXMLToList<T>(string xml)
        {
            if (String.IsNullOrEmpty(xml)) throw new NotSupportedException("ERROR: input string cannot be null.");
            try
            {
                var xmlserializer = new XmlSerializer(typeof(List<T>), new XmlRootAttribute("root"));
                var stringReader = new StringReader(xml);
                using (var reader = XmlReader.Create(stringReader))
                {
